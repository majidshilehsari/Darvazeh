...
class Store:
    def __init__(self, root):
        ...
        if existed and self.db.execute('PRAGMA user_version').fetchone()[0] == 0:
            backup = self.root / 'history-before-upgrade.sqlite3'
    ...
    def cleanup(self):
        with self.lock, self.db:
            ...
            snapshot = self.root / 'history-before-upgrade.sqlite3'
            if snapshot.exists() and snapshot.stat().st_mtime < cutoff:
                snapshot.unlink()
...
class Relay:
    def __init__(self, store, devices, trusted='', country_path='/data/GeoLite2-Country.mmdb', port=10001, target=10000):
    ...
    async def handle(self, reader, writer):
        if self.connections>=256:
            writer.close(); return
        self.connections+=1
        upstream=None; tasks=[]; key=None; counts=[0,0]
        try:
            ...
            prefix=Prefix()
            # Early-data is forwarded but deliberately not attributed to a device.
            if headers.get('sec-websocket-protocol'): prefix.done=True
            started=time.monotonic()
            async def pump(src,dst,direction):
                nonlocal key
                while True:
                    chunk=await asyncio.wait_for(src.read(65536),3600)
                    if not chunk: return
                    counts[direction]+=len(chunk)
                    if direction==0 and key is None:
